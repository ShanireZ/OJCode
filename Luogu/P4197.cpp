...
Node ns[MX * 20];
int read(), n, m, q, kh, epos, spos, cnt, npos, ls[MX], h[MX], g[MX], dp[MX], anc[MX][20];
...
{
    dp[now] = deep, kh = max(kh, deep), sid[now] = 0x3f3f3f3f;
    if (val[now] == 0) // 叶节点
    {
...
int main()
{
    ...
    for (int i = 1; i <= q; i++)
    {
        ...
        printf("%d\n", ls[query(root[r], root[l - 1], 1, cnt, sz[pos] - k + 1)]);
    }
    return 0;
}
int read()
