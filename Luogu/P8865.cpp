...
int main()
{
    ...
    while (t--)
    {
        ...
        cin >> n >> m >> c >> f;
        memset(hz, -1, sizeof(hz)), memset(thz, 0, sizeof(thz)), memset(thhz, 0, sizeof(thhz)), memset(h, 0, sizeof(h));
        char ch;
        for (int i = 1; i <= n; i++)
        {
            for (int j = 1; j <= m; j++)
        ...
        for (int i = n; i >= 1; i--)
        {
            for (int j = m; j >= 1; j--)
            {
                ...
                hz[i][j] = hz[i][j + 1] + 1;
                if (i + 2 <= n && a[i + 1][j] + a[i + 2][j] == 0)
                {
                    thz[i][j] = thz[i + 1][j] + hz[i + 2][j];
                }
                if (i + 3 <= n && a[i + 1][j] + a[i + 2][j] + a[i + 3][j] == 0)
                {
                    h[i][j] = h[i + 1][j] + 1;
